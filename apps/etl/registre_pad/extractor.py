...
def extraire_quais_reference(fichier) -> pd.DataFrame:
    """
    Lit l'onglet AUTRES DONNEES et retourne la correspondance POSTE -> SPECIALITE
    (utilisée comme proxy du couple QUAI/TERMINAL en l'absence d'un référentiel dédié).
    """
    df = pd.read_excel(fichier, sheet_name=ONGLET_AUTRES, engine="openpyxl", header=None, skiprows=2, dtype=str)
    quais = df.iloc[:, [0, 1]].copy()
    quais.columns = ["specialite", "poste"]
    quais = quais[quais["poste"].notna() & (quais["poste"].astype(str).str.strip() != "")]
    quais["specialite"] = quais["specialite"].where(quais["specialite"].notna(), "Non spécifié")
    return quais.reset_index(drop=True)
